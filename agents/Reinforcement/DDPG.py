...
CURRICULUM_CONFIG = {
    # Each level can now specify track geometry: 'track_width' and 'curves' (number of turns).
    # These values will be applied to any Racer instance used for training/evaluation.
    'level_0': {
        'name': 'Easy',
        'obstacles': False,
        'chicanes': False,
    ...
    'level_1': {
        ...
        'track_width': 0.08,
        'curves': 8,
        'min_success_rate': 0.90,  # 80% completion rate to advance
        'min_steps': 5000,          # Minimum exploration time before evaluation
        'eval_episodes': 10         # Episodes to test mastery
    },
    'level_2': {
        ...
        'track_width': 0.06,
        'curves': 12,
        'min_success_rate': 0.90,   # 70% completion rate to advance
        'min_steps': 8000,
        'eval_episodes': 50
    },
    'level_3': {
...
if TRAINING_MODE == 'curriculum':
    ...
    # Apply geometry parameters from curriculum level
    if 'track_width' in initial_config:
        racer.track_width = initial_config['track_width']
    if 'curves' in initial_config:
        racer.curves = initial_config['curves']
...
gamma = float(os.getenv('GAMMA', 0.99))  # Slightly higher discount factor
tau = float(os.getenv('TAU', 0.005))  # Faster target network updates
critic_lr = float(os.getenv('CRITIC_LR', 0.0003))  # Equal learning rates
actor_lr = float(os.getenv('ACTOR_LR', 0.0003))
...
buffer_dim = int(os.getenv('BUFFER_DIM', 100000))
batch_size = int(os.getenv('BATCH_SIZE', 64))  # Increased from 32
# Default warmup before training starts (transitions)
warmup_steps = 2000
# Allow overriding warmup and total iterations from environment for dev runs
warmup_steps = int(os.getenv('WARMUP_STEPS', warmup_steps))

# Noise parameters from env
noise_scale = float(os.getenv('NOISE_SCALE', 0.15))
noise_accel_mult = float(os.getenv('NOISE_ACCEL_MULT', 1.0))
noise_steer_mult = float(os.getenv('NOISE_STEER_MULT', 1.5))

# Safe warmup config (use a conservative hand-coded policy during warmup)
SAFE_WARMUP_ENABLED = str(os.getenv('SAFE_WARMUP', '0')).lower() in ('1', 'true', 'yes')
SAFE_WARMUP_ACCEL = float(os.getenv('SAFE_WARMUP_ACCEL', 0.4))
SAFE_WARMUP_STEER_STD = float(os.getenv('SAFE_WARMUP_STEER_STD', 0.03))
SAFE_WARMUP_STEER_CLIP = float(os.getenv('SAFE_WARMUP_STEER_CLIP', 0.12))
_safe_warmup_printed = False

# Training configuration
QUICK_TEST = False
total_iterations = int(os.getenv('TOTAL_ITERATIONS', 5000 if QUICK_TEST else 50000))
...
def get_actor(train_acceleration=True, train_direction=True):
    ...
    out1 = layers.Dense(1, activation='tanh', trainable=train_acceleration,
                        bias_initializer=keras.initializers.Constant(0.2))(out1)
    ...
    out2 = layers.Dense(1, activation='tanh', trainable=train_direction,
                        bias_initializer=keras.initializers.Constant(0.0))(out2)
...
class Buffer:
    ...
    def sample_batch(self, *args, **kwargs):
        # Accept extra args to be robust when called from TF-autograph-wrapped
        # contexts (some Keras/TF internals may pass extra positional args).
...
def policy(state, current_iteration=0, max_iterations=50000, verbose=False):
    ...
    # If safe warmup is enabled, during warmup use a conservative policy
    global _safe_warmup_printed
    if SAFE_WARMUP_ENABLED and current_iteration < warmup_steps:
        if not _safe_warmup_printed:
            print(f"⚠️ SAFE WARMUP enabled: using conservative policy for first {warmup_steps} steps")
            _safe_warmup_printed = True
        # forward acceleration with tiny steering noise
        a = SAFE_WARMUP_ACCEL + np.random.normal(scale=0.02)
        s = np.random.normal(scale=SAFE_WARMUP_STEER_STD)
        s = np.clip(s, -SAFE_WARMUP_STEER_CLIP, SAFE_WARMUP_STEER_CLIP)
        sampled_action = np.array([a, s], dtype=np.float32)
    ...
    # Base noise scale (configurable)
    noise = np.random.normal(scale=noise_scale * decay_factor, size=2)

    # FIXED: Better noise balance - steering normally gets more exploration,
    # but during initial warmup reduce steering noise to avoid immediate wrong-direction failures.
    steer_multiplier = noise_steer_mult
    if current_iteration < warmup_steps:
        steer_multiplier = max(0.15, noise_steer_mult * 0.4)

    noise[0] *= noise_accel_mult   # Acceleration noise
    noise[1] *= steer_multiplier   # Steering noise (reduced during warmup)
    ...
    legal_action = np.clip(sampled_action, lower_bound, upper_bound)
    # return as flat numpy array [accel, steer]
    return np.squeeze(legal_action)
...
if __name__ == '__main__':
    ...
    def evaluate_policy(actor, num_episodes=10, verbose=False, eval_difficulty='hard', current_level=1):
        ...
        if eval_difficulty == 'current' and TRAINING_MODE == 'curriculum':
            ...
            # Apply curriculum geometry if provided
            if 'track_width' in config:
                eval_racer.track_width = config['track_width']
            if 'curves' in config:
                eval_racer.curves = config['curves']
        else:
    ...
    def train(total_iterations=total_iterations, training_start_time=None):
        ...
        max_curriculum_level = max([int(k.split('_')[1]) for k in CURRICULUM_CONFIG.keys()])
        ...
        ep = 0
        avg_reward = 0
        # Episode bookkeeping for debug: iteration index at last episode boundary
        last_episode_iter = 0
        ...
        # Helper to decide if episode debug prints should be enabled
        def _debug_enabled():
            v = os.environ.get('DEBUG_EPISODES')
            if v is None:
                return False
            v = str(v).strip().lower()
            return v in ('1', 'true', 'yes') or (v != '0' and v != '')
        ...
        while i < total_iterations:
            ...
            while not(done):
                ...
                action = policy(tf_prev_state, current_iteration=i, max_iterations=total_iterations)
                ...
                # TRAINING WITH WARMUP (start when we have at least one minibatch or reach warmup)
                if buffer.buffer_counter >= max(batch_size, warmup_steps):
                    if not warmup_done_printed:
                        print(f"🔥 Warmup complete at iteration {i} (buffer={buffer.buffer_counter}) — training enabled")
                ...
                if (TRAINING_MODE == 'curriculum' and 
                    current_curriculum_level < max_curriculum_level and 
                    ...
                    if steps_in_level >= current_config['min_steps']:
                        ...
                        if success_rate >= current_config['min_success_rate']:
                            ...
                            # Apply curriculum geometry to new racer
                            if 'track_width' in config:
                                racer.track_width = config['track_width']
                            if 'curves' in config:
                                racer.curves = config['curves']
                            ...
                            # Debug: print episode length and termination reason if enabled
                            try:
                                comp = racer.completation
                            except Exception:
                                comp = None

                            # Map completion codes to human-readable reasons (tracks.py semantics)
                            comp_map = {1: 'completed', 2: 'off-road', 3: 'wrong-direction', 4: 'under-speed', None: 'unknown'}
                            comp_str = comp_map.get(comp, f'code-{comp}')

                            ep_len = i - last_episode_iter
                            last_episode_iter = i

                            # Only print lightweight debug lines to avoid huge logs; guard by env var
                            if _debug_enabled():
                                print(f"[EP_DEBUG] ep={ep} steps={ep_len} termination={comp_str} (code={comp}) reward={reward:.2f} mean_speed={mean_speed:.3f}")
                            break
                        else:
            ...
            # report termination code and episode length to help debug single-step failures
            try:
                term_code = racer.completation
            except Exception:
                term_code = None
            ep_len = i - last_episode_iter
            print("Episode {}: Iterations {}, Avg. Reward = {:.2f}, Last reward = {:.2f}, term={}, ep_steps={}, Avg. speed = {:.3f}".format(
                ep, i, avg_reward, episodic_reward, term_code, ep_len, mean_speed/i))
            print("\n")

            # Per-episode debug: report termination reason and episode length
            try:
                comp = racer.completation
            except Exception:
                comp = None
            ...
            last_episode_iter = i
            if _debug_enabled():
                print(f"[EP_DEBUG] ep={ep} steps={ep_len} termination={comp_str} (code={comp}) episodic_reward={episodic_reward:.2f} mean_speed={mean_speed/i:.3f}")
